...
Scholar,\footnote{\url{https://www.semanticscholar.org/}} but find no
...
below, the committees acted better than random, but not
...
23.5\% of the time. As such the committees acted better than
random, though maybe not as significantly as some may have hoped for (see also Appendix~\ref{random-committee-outcomes}).
...
Particular public reaction triggered by a blog post from Eric Price.\footnote{\url{http://blog.mrtz.org/2014/12/15/the-nips-experiment.html}} where much of the discussion speculates on the number of consistent accepts in
the process (using the main conference accept rate as a proxy). As a result the analysis
 produces numbers that don't match ours above (see also Appendix~\ref{app:neurips-experiment-results}). This is because
...
duplicated papers, but as we show in Appendix~\ref{uncertainty-accept-rate} this difference falls well within statistical sampling error. We look at these
...
differently by different reviewers. Since at least 2005 conference, NeurIPS
...
\cite{Ge-bayesian15}. See also a more recently proposed approach to calibration from \cite{MacKay-calibration17}Corinna Cortes and Neil Lawrence also used a
...
\citep{Gpy-2012}. The parameters of the fitted model are given in
...
\begin{figure}[htb]
  \begin{center}
    \includegraphics[width=\columnwidth]{diagrams/neurips/citations-vs-average-calibrated-quality-accept.pdf}
  \end{center}
...
\begin{figure}[htb]
  \begin{center}
    \includegraphics[width=\columnwidth]{diagrams/neurips/citations-vs-average-impact-accept.pdf}
  \end{center}
  \caption{Scatter plot of the citation impact (defined as
...
\begin{figure}[htb]
\includegraphics[width=\columnwidth]{diagrams/neurips/citations-vs-average-confidence-accept.pdf}
...
was was $0.39 \pm 0.072$, giving additional evidence for
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=\columnwidth]{diagrams/neurips/citations-vs-average-calibrated-quality-reject.pdf}
\end{center}
\caption{Scatter plot of $\log_{10}(1+\text{citations})$ against the
