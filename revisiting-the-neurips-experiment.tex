...
\twocolumn[
\aistatstitle{Inconsistency in Conference Peer Review: Revisiting the 2014 NeurIPS Experiment}
...
inconsistency in conference peer review. We determine that \ignore{the
...
papers were selected to appear at the conference, see Table~\ref{table-neurips-experiment-results}. We call this
...
in supervised and unsupervised learning, structure prediction, etc., providing theoretical, algorithmical and experimental justification for the proposed techniques.\ignore{unsupervised learning with neural networks as well as papers on
...
Each selected paper went through the review process independently. Authors were notified if their paper was in the experiment as they would have to write two independent author rebuttals.

The results of this process are summarised in Table
\ref{table-neurips-experiment-results}. For the final conference a paper was accepted if any of the committees would have accepted it. As the table illustrates, the two committee only had a 50\% agreement on what papers to accept.
\begin{table}[htb]
...
Cortes and Lawrence also looked at the correlation between calibrated review
scores across the two independent committees, a scatter plot of the
\cc{mean?} scores from the committees is shown in Figure~\ref{figure-calibrated-quality-correlation}, the Pearson correlation
...
sensible starting point to unpick \cc{??, uncover?} the inconsistency between the two
...
Ghaharamani introduced a separate scoring indicator (see Appendix~\ref{impact-score}). Perhaps the answer to our quandry in the quality
