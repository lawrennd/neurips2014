...
\ignore{To run the experiment, we created two separate committees within the
NeurIPS program committee. The idea was that the two separate committees
...
\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{with} \BuiltInTok{open}\NormalTok{(os.path.join(NeurIPSy.review\_store, NeurIPSy.outlet\_name\_mapping), }\StringTok{\textquotesingle{}r\textquotesingle{}}\NormalTok{) }\ImportTok{as}\NormalTok{ f:}
...
\NormalTok{citations }\OperatorTok{=}\NormalTok{ NeurIPSy.load\_citation\_counts(date}\OperatorTok{=}\NormalTok{date)}
\NormalTok{decisions }\OperatorTok{=}\NormalTok{ NeurIPSy.load\_decisions()}
\NormalTok{NeurIPSy.augment\_decisions(decisions)}
\NormalTok{joindf }\OperatorTok{=}\NormalTok{ NeurIPSy.join\_decisions\_citations(decisions, citations)}
...
\ControlFlowTok{if}\NormalTok{ UPDATE\_IMPACTS:}
\NormalTok{    semantic\_ids }\OperatorTok{=}\NormalTok{ NeurIPSy.load\_semantic\_ids()}
...
\NormalTok{    sscholar }\OperatorTok{=}\NormalTok{ NeurIPSy.download\_citation\_counts(citations\_dict}\OperatorTok{=}\NormalTok{citations\_dict, semantic\_ids}\OperatorTok{=}\NormalTok{semantic\_ids)}
